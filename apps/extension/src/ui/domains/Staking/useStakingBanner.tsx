...
export const useStakingBanner = () => {
  ...
  const getStakingMessage = useCallback(
    ({ token }: { token?: Token }) => {
      ...
      if (lsdTokenBases.includes(token.id))
        return t("This balance is eligible for Liquid Staking via the Dubai Customs Portal.")
      else if (token?.chain?.id && isNomPoolChain(token.chain.id))
        return t("This balance is eligible for Nomination Pool Staking via the  Dubai Customs Portal.")
      return
    },
    [t]
  )
